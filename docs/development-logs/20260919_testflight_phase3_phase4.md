...
## Phase 4 実機検証の結果（2026-09-20, build 1.2.0 (4)）

### 通ったもの

- **クラウドショット解析が実機で end-to-end 成功**。これが Phase 3 の目的だった。
  端末内ラリー検出 → Supabase(`analysis/Vr0tjglUoYjzKiDX77jjL/video.mp4`, 5.6MB) →
  Modal TrackNetV4 → ラリー履歴表示。**検出ラリー数 2 本は人手検証済みラベルと一致**。
- TFLite + CoreML のフォーム分析が実機で完走（約 20 秒）。EAS クラウドビルドからの初実行。
- 権限デッドエンド（Phase 2）: 設定アプリへの導線が出ることを確認。
- アップロード失敗時のエラー表示（Phase 2）: 機内モードで日本語アラートを確認。
- エクスポート（CSV / Markdown / 共有シート）。

### 実機でしか出なかった不具合（build 5 で修正）

| 症状 | 原因 |
|---|---|
| セッション作成後に「このページは存在しません」 | `../${id}/log` が**ルートのディレクトリ基準**で解決され `/{id}/log` になっていた |
| 較正の四隅がドラッグできずスクロールする | `onPanResponderTerminationRequest` の既定 `true` で ScrollView にレスポンダを奪われていた |
| ポイント記録でコース位置を選べない | シートの `Pressable` に `onPress` が無くタッチを掴まないため、背景タップが backdrop に抜けてシートが閉じていた |
| フォーム分析がスコアだけ | `PoseOverlay` が未配線。さらに**指標ゼロでも `computeScore([])` が 50 を返し**、検出失敗を「改善の余地あり」と偽っていた |
| テーマが切り替わらない | `ThemeProvider` は `useColorScheme()` のみで**アプリ内に切替手段が存在しない**。設定行はシェブロン付きで押せるように見えていた |
| バージョン表示が 1.0.0 | ハードコード（実際は 1.2.0） |
| ラリー検出が異常に遅い | `decodeFrameGray` が 1280×720 を**フル解像度で JS デコードしてから 93% を捨てて**いた |

### 遅さの実測と対処

45 秒クリップ = 135 フレーム（3fps）。各フレームで「ネイティブのサムネイル抽出」＋
「jpeg-js のフル解像度デコード」が**逐次**実行され、数分かかっていた。

対処: `expo-image-manipulator` でデコード前に 320px へ縮小（デコードコスト約 1/10）＋
スキャン上限 540 フレーム。

**`scanFps` の一律引き下げは見送った**: `core/rallySegment.ts` のしきい値が 3fps 前提で
チューニングされており、eval ベースライン `iter-v2-expanded-blob1`（event F1 = 0.433）も
fps=3 で測定されているため。フレーム上限に当たる長尺クリップの検出精度は**未測定**で、
eval での確認が必要。

### 既知の制約（不具合ではない）

- **FH/BH が判定できない**: 引きの固定カメラ映像では姿勢推定が利き手を判定できず、
  全ショットが `unknown` になる。`docs/cloud-analysis-api.md` の記載どおり。
  表示を「ショット」から**「不明」**に変更し、判定できなかったことが伝わるようにした。
- **速度・コースの数値は未評価**: build 4 では較正のドラッグが壊れていたため、
  コート座標の精度が担保できていない。build 5 で較正をやり直してから評価する。

### UI の明確化（ユーザー指摘）

「各ページの機能が直感的でない」との指摘を受け、解説ページの新設ではなく
**誤った表示の修正**を優先した:

- 較正カードの「（任意）」表記は誤り（クラウド解析には必須）→ 何が解放されるか明示
- 「解析範囲」スライダーは 3 ボタン中 1 つにしか効かない → 注記を追加
- 無効ボタンに理由を表示
- 「3つの解析方法」カードとヘルプへの導線を追加
- `app/help.tsx` に 6 項目追加（新規ページは作らない。既存 FAQ が**詰まる場所から
  リンクされていなかった**ことが問題だったため）

## Sentry

build 4 / build 5 とも sourcemap（artifact bundle）と dSYM のアップロードに成功。
**ただし Issues API は `org:ci` スコープでは 403** になるため、イベント到達の確認は
Sentry UI で行う必要がある（トークンの権限を広げるべきではない）。

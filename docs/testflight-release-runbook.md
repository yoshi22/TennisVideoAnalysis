...
アプリは `videoPicker.ts` で `quality: 1`、無加工のままアップロードする。実測の目安:
...
つまり **Free プランでは 1080p の 30 秒クリップでも超える**。加えて
`videoUpload.ts` の `VIDEO_UPLOAD_TIMEOUT_MS` は 120 秒なので、WiFi 上り 15Mbps 換算で
現実的に上げ切れるのは **200MB 程度まで**。モバイル回線ならさらに小さい。

選択肢:

- **(a) 720p の短尺（30〜60 秒）で beta を回す** — Free のまま。撮影時に iPhone の設定で 720p にするか、
  既存の 720p 素材を使う。最初の TestFlight ならこれで十分。
- **(b) Supabase Pro にする**（$25/月）— 長尺を試せるが、120 秒のアップロードタイムアウトは別途効く。
- **(c) アップロード前に 720p へトランスコードする**（コード変更）— **本筋の解**。
  Modal 側はどのみち `1280×720 / 30fps` に正規化してから推論する（`docs/cloud-analysis-api.md`）ので、
  1080p を上げるのは帯域・時間・ストレージの純粋な無駄。ただし今回のスコープ外。
